...
2. **Check for existing branches before creating new one**:
   ...
   d. Run the script `.specify/scripts/powershell/create-new-feature.ps1 -Json "$ARGUMENTS"` with the calculated number and short-name. If the script output indicates that spec.md already exists in the feature directory, read and update the existing spec rather than writing from scratch. If SPEC_FILE already contains content beyond the template, you are updating an existing spec — preserve existing sections and only modify what the user requested:
