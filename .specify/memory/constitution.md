<!--
  Sync Impact Report
  ==================================================
  Version change: 1.1.1 → 1.1.2 → 1.2.0
  Modified principles: I (clarified)
  Added sections:
  ...
  Feature artifacts requiring updates:
    - specs/001-speckit-repo-context/plan.md ✅ updated
      (Constitution Check table updated from 6 to 7 principles
      to include VII. DRY)
  Follow-up TODOs: none
  ==================================================
-->

# Speckit-custom Constitution
...
- **Editor**: VS Code is the primary and only supported editor.
- **Context delivery**: `.github/agents/` for always-on agent
  instructions; `.github/prompts/` for stage-specific slash
  commands.
- **Scripts**: PowerShell (cross-platform).
- **Content**: Markdown for all prompts, templates, and
  specifications.
- **Storage**: File-based only (`.github/`, `.specify/`,
  `specs/`). No database or backend services.
- **Configuration**: `.specify/config.json` (JSON format) for
  path overrides. Environment variables as fallback.
- **Agent skills**: Skill modules (for example in
  `.claude/skills/` or equivalent configured skill packs) are
  in-scope enforcement options when they codify repeatable
  principle checks.
- **Tools**: Agent-accessible tools (search, file edit,
  validation, and terminal/script execution) are in-scope for
  enforcing constitution principles when used via prompt and
  agent instructions.
- **Workflow hooks**: Pre-stage checks, post-stage validation,
  and update hooks in `.specify/scripts/` and Speckit command
  prompts are in-scope enforcement points.

## Development Workflow

- **Stage progression**: specify → plan → clarify → tasks → analyze → implement → verify → cleanup.
  Each stage has a dedicated prompt and template.
- **Required context**: Each stage MUST load stage-appropriate
  context before the agent writes output. Always-on context
  MUST stay under 200 lines of Markdown source.
- **Repeatability**: The same commands and templates MUST be
  used each time a new spec is started. The process MUST be
  deterministic in structure (not in content).
- **Core/custom boundary**: Core speckit files use
  `<`
  markers to separate repo-specific customizations from
  updatable core content.
- **Version tracking**: `.specify/version` MUST contain a
  version string and core file manifest with checksums.

## Governance

This constitution supersedes all other development practices
for this repository. Amendments require:

1. **Proposal**: Describe the change and its rationale in a
   specification or discussion.
2. **Review**: Validate the amendment does not contradict
   existing principles without explicitly superseding them.
3. **Documentation**: Update the constitution file, increment
   the version per semantic versioning (MAJOR for principle
   removals/redefinitions, MINOR for new principles/sections,
   PATCH for clarifications), and update the amendment date.
4. **Propagation**: After any amendment, verify all dependent
   templates (plan, spec, tasks) and agent/prompt files still
   align with the updated principles.

All specification reviews MUST verify compliance with these
principles. Complexity beyond what the specification requires
MUST be justified in the plan's Complexity Tracking section.

**Version**: 1.2.0 | **Ratified**: 2026-02-28 | **Last Amended**: 2026-03-08
